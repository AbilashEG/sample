...
def list_cves():
    ...
    filter_cve_id = request.args.get('filterCveId', '').strip()
    filter_cvss_score = request.args.get('filterCvssScore', '')
    filter_date_modified = request.args.get('filterDateModified', '')

    # Pagination and sorting parameters
    page = int(request.args.get('page', 1))
    results_per_page = int(request.args.get('resultsPerPage', 10))
    sort_order = request.args.get('sort', 'published_date')
    ...
    # Construct query dynamically with filtering options
    query = """
    ...
    if filter_cve_id:
        ...
        params.append(f"%{filter_cve_id}%")

    if filter_cvss_score:
        query += " AND cvss_v2_score >= %s"
        params.append(filter_cvss_score)

    if filter_date_modified:
        query += " AND modified_date >= %s"
        params.append(filter_date_modified)

    query += f" ORDER BY {sort_order} {sort_direction} LIMIT %s OFFSET %s"
    params.extend([results_per_page, start_index])

    # Connect to database and execute query
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT COUNT(*) AS total FROM cve_details WHERE 1=1", params[:len(params)-2])  # Count records without LIMIT/OFFSET
